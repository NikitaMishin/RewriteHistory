...
public class BezierCurve : MonoBehaviour {
	...
	public List<Vector3> GetAllPointsAlongCurve ()
	{
		var allPoints = close ? 
			new List<Vector3>(resolution*(points.Length-1)): new List<Vector3>(resolution*(points.Length));

		if(points.Length > 1){
			allPoints.Add(points[0].position);
			for(int i = 0; i < points.Length - 1; i++)
			{
				allPoints.AddRange(GetPointsCurve(points[i], points[i+1], resolution));
			}
			
			if (close) allPoints.AddRange(GetPointsCurve(points[points.Length - 1], points[0], resolution));
			return allPoints;
		}
		else
		{
	...
	// return point in curve  all points except initial
	// size of array =resolution
	public static List<Vector3> GetPointsCurve(BezierPoint p1, BezierPoint p2, int resolution)
	{
		...
		Vector3 lastPoint = p1.position;
		Vector3 currentPoint = Vector3.zero;
		List<Vector3> pointsCurve = new List<Vector3>(resolution);

		for(int i = 1; i < limit; i++){
			currentPoint = GetPoint(p1, p2, i/_res);
			lastPoint = currentPoint;
			pointsCurve.Add(lastPoint);
		}
		return pointsCurve;
	}
